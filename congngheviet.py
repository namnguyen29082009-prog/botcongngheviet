...
# ================== CẤU HÌNH ==================
TOKEN = os.getenv("TOKEN")  # FIXED: dùng biến môi trường
OWNER_ID = 8605217948
...
def get_api():
    ...
    """
    try:
        r = requests.get("https://sunlaymaynkx.hacksieucap.pro/sunlon", timeout=10)
        r.raise_for_status()
        ...
        phien = js.get("Phien") or js.get("phien_sex") or js.get("phien")
        ...
        tong = js.get("Tổng") if "Tổng" in js else js.get("Tong") or js.get("total") or js.get("tong")
...
/start /stop /stopsicbosun /sicbosun - Bật,Tắt dự đoán Sun
        GAME HIT CLUB
...
def sicbosun_auto(uid, chat_id):
    ...
    while uid in running_users:
        try:
            ...
            r = requests.get("https://sicsun-6lpf.onrender.com/predict", timeout=5)
            ...
            if not phien or (uid in user_data and user_data[uid].get("last_phien") == phien):
                time.sleep(3)
                continue
            ...
            tong = data.get("Tong") or data.get("Tổng") or data.get("total")
...
def sicbosun_cmd(msg):
    ...
    if uid in running_users:
        bot.reply_to(msg, "Sicbo Sun đang chạy")
        return

    bot.reply_to(msg, "Bắt đầu dự đoán Sicbo Sun")
...
def stopsicbosun_cmd(msg):
    ...
    if uid not in running_users:
        bot.reply_to(msg, "Bạn chưa chạy Sicbo Sun hoặc đã dừng rồi.")
        return

    running_users.discard(uid)
    bot.reply_to(msg, "Dừng dự đoán Sicbo Sun.")
# ===== QUẢN LÝ TRẠNG THÁI GAME =====
game_running = {}
game_last_phien = {}
running_users = set()  # lưu user đang chạy
...
def hitxanh_auto(uid, chat_id):
    ...
    while uid in running_users:
        try:
            ...
            r = requests.get("https://hitclub-ovh1.onrender.com/api/taixiu", timeout=5)
            if r.status_code != 200:
                time.sleep(3)
                continue
            ...
            if current_time - last_error_time > 60:
                ...
                last_error_time = current_time
            if error_count > 10:
                print(f"Dừng auto_loop Hit Xanh cho user {uid} do quá nhiều lỗi")
                running_users.discard(uid)
                break
...
def hitxanh_cmd(msg):
    ...
    if uid in running_users:
        bot.reply_to(msg, "Hit Xanh đang chạy")
        return

    bot.reply_to(msg, "Bắt đầu dự đoán Hit Xanh")
...
def stophitxanh_cmd(msg):
    ...
    if uid not in running_users:
        bot.reply_to(msg, "Bạn chưa chạy Hit Xanh hoặc đã dừng rồi.")
        return

    running_users.discard(uid)
    bot.reply_to(msg, "Dừng dự đoán Hit Xanh.")
# ===== QUẢN LÝ TRẠNG THÁI GAME =====
game_running = {}
game_last_phien = {}
running_users = set()  # lưu user đang chạy
...
def hitmd5_auto(uid, chat_id):
    ...
    while uid in running_users:
        try:
            ...
            r = requests.get("https://hitclub-ovh1.onrender.com/api/taixiumd5", timeout=5)
            if r.status_code != 200:
                time.sleep(3)
                continue
            ...
            if current_time - last_error_time > 60:
                ...
                last_error_time = current_time
            if error_count > 10:
                print(f"Dừng auto_loop Hit MD5 cho user {uid} do quá nhiều lỗi")
                running_users.discard(uid)
                break
...
def hitmd5_cmd(msg):
    ...
    if uid in running_users:
        bot.reply_to(msg, "Hit MD5 đang chạy")
        return

    bot.reply_to(msg, "Bắt đầu dự đoán Hit MD5")
...
def stophitmd5_cmd(msg):
    ...
    if uid not in running_users:
        bot.reply_to(msg, "Bạn chưa chạy Hit MD5 hoặc đã dừng rồi.")
        return

    running_users.discard(uid)
    bot.reply_to(msg, "Dừng dự đoán Hit MD5.")
# ===== QUẢN LÝ TRẠNG THÁI GAME =====
game_running = {}
game_last_phien = {}
running_users = set()  # lưu user đang chạy
...
def b52md5_auto(uid, chat_id):
    ...
    while uid in running_users:
        try:
            ...
            r = requests.get("https://b52-cbqn.onrender.com/api/taixiumd5", timeout=5)
            if r.status_code != 200:
                time.sleep(3)
                continue
            ...
            if current_time - last_error_time > 60:
                ...
                last_error_time = current_time
            if error_count > 10:
                print(f"Dừng auto_loop B52 MD5 cho user {uid} do quá nhiều lỗi")
                running_users.discard(uid)
                break
...
def b52md5_cmd(msg):
    ...
    if uid in running_users:
        bot.reply_to(msg, "B52 MD5 đang chạy")
        return

    bot.reply_to(msg, "Bắt đầu dự đoán B52 MD5")
...
def stopb52md5_cmd(msg):
    ...
    if uid not in running_users:
        bot.reply_to(msg, "Bạn chưa chạy B52 MD5 hoặc đã dừng rồi.")
        return

    running_users.discard(uid)
    bot.reply_to(msg, "Dừng dự đoán B52 MD5.")
# ===== QUẢN LÝ TRẠNG THÁI GAME =====
game_running = {}
game_last_phien = {}
running_users = set()  # lưu user đang chạy
